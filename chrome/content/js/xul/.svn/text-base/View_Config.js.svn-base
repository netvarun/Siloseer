...
function updateUI()
{
    var wrapperId = domains[global_select.domain]['id'];
    var usermd5  = MD5(loginVar.userName);
    var crawlUrl = serverUrl+'runCrawl.pl?'+'u='+usermd5+'&wId='+wrapperId;
    var dom_name = document.getElementById("Edit_Domain_Crawl_Button");
    dom_name.childNodes[0].removeAttribute("oncommand");
    dom_name.childNodes[0].setAttribute("oncommand","mainWindow.content.wrappedJSObject.location='"+crawlUrl+"'");
}
